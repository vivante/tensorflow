...
class HloToMemrefRngGetAndUpdateStateConverter
 ...
 public:
  ...
  LogicalResult matchAndRewrite(
    ...
    int word_size = result_type.getElementType().getIntOrFloatBitWidth();
    auto smaller_int_type = rewriter.getIntegerType(word_size);
    int num_elements = result_type.getNumElements();
    ...
    Value delta = rewriter.create<arith::ConstantOp>(
        loc, rewriter.getIntegerAttr(seed_type, adaptor.delta()));
